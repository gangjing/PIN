...
公开分享站点地址：`https://gangjing.github.io/PIN/`

完整组合报告默认只生成在本地 `output/` 和 `reports/`。GitHub Pages 默认只发布经过清洗的单股分享页。
...
- `privacy.hide_sensitive_fields` 控制完整报告是否隐藏数量、成本、仓位
- `public_site.publish_full_report` 控制是否把完整组合报告发布到 GitHub Pages，默认 `false`
- `public_site.publish_stock_pages` 控制是否生成单股公开分享页，默认 `true`
- `public_site.show_share_index` 控制是否公开单股目录页，默认 `false`
...
- `reports/market_report_YYYYMMDD_HHMM.html`
- `output/share_links.md`
- `docs/share/{TICKER}.html`

`output/latest_report.html` 是单文件报告，可以直接双击或用浏览器离线打开。

## 单股分享和隐私

默认公开网站不会发布完整组合报告。每天运行后会为每只股票生成一个单独分享页：

```text
https://gangjing.github.io/PIN/share/1810.HK.html
```

分享页只包含这只股票的行情、技术指标、新闻和分析，不包含你的完整股票池，也不包含数量、成本、仓位、止盈止损、关注理由、个人备注等字段。所有可分享链接会写入：
...
**是否会隐藏真实持仓？**  
默认 `hide_sensitive_fields: true`，并隐藏 `quantity`、`cost`、`position_pct`。公开单股分享页还会额外移除止盈止损、关注理由和个人备注。

**别人能不能看到我的完整 Google Sheet 信息？**  
不能通过默认公开页面看到。Google Sheet 仍然只作为运行时数据源；公开 Pages 默认只保留清洗后的单股页面。如果把完整报告发布开关改成 `true`，完整报告会变成公开内容。
